...
Function Create-AzEnvironment($resourceGroupName, $location, $location2) {
  ...
  Add-AzVirtualNetworkPeering `
    ...
    -RemoteVirtualNetworkId $virtualNetwork2.Id |Out-Null
...
Function Create-CustomVM ($VMName, $location, $resourceGroupName, $availabilitySetName, $VirtualNetwork, $adminuser){
  ...
  $nsg = New-AzNetworkSecurityGroup -ResourceGroupName $resourceGroupName `
    ...
    -SecurityRules $rule1
  Write-Host "SubnetId $($virtualNetwork.Subnets[0] | Select -ExpandProperty ID)"
...
$results[$vm4] = Create-CustomVM -VMName $vm4 -location $location2 -resourceGroupName $resourceGroupName -VirtualNetwork $virtualnetwork2 -adminuser $adminuser -availabilitySetName 'Elk'
...
while($i -lt 4){
  ...
  $status4 = $stats | where {$_.name -eq $vm1}
  if($status4.ProvisioningState -eq 'Succeeded'){$i++}
  start-sleep 5
}
